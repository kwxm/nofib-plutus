module ChessSetArray
where

-- We don't have arrays in PLC at present, so let's ignore this for now.
...
data ChessSet = Board Integer Integer Tile Tile (Array Integer Integer)
...
tileIndex:: Integer -> Tile -> Integer
tileIndex size (x,y) = ((x-1)*size) + y

indexTile::Integer -> Integer -> Tile
